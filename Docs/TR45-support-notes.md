...
- 通知特性 (Data Line RX): 実際に notify フラグを持つ `6E400004-...` / `6E400006-...` / `6E400005-...` を優先し、全 notify を enable して取りこぼしを防ぎます。
- 書き込み特性 (Data Line TX): `6E400003-...`（writeNR）で応答が安定する個体があるため最優先し、次に `6E400002-...`、`6E400007-...` を順に試します。
- `ConnectionManager` はサービス内の全 characteristic を取得し、**通知系はすべて setNotifyValue(true) で有効化**。優先順は notify が `0x0004`→`0x0006`→`0x0005`、write は `0x0003`→`0x0002`→`0x0007`→プロファイル指定の順で探し、プロパティが一致するものへフェールバックします。応答がない場合はポーリング側が自動で write を次候補へ切り替えます。
...
## 登録コード(0x76)によるロック解除
- TR4A 本体にパスコードロックがある場合、0x33 応答ステータスが `0x0F (REFUSE)` になる。**セキュリティ OFF 機に対してはいきなり 0x76 を送らず、まず 0x33 を投げて REFUSE を見てから 0x76 を送る** フローに変更済み。
- Settings > Bluetooth に登録コード入力欄を追加。**印字されている 8 桁の数字列を「10進8桁の BCD 4byte」**として解釈し、上位桁から詰めて 0x76 に埋め込む。
  - 例: `74976167` → BCD `74 97 61 67` → フレーム `01 76 00 04 00 74 97 61 67 32 8C`（CRC16/XMODEM の結果は 0x8C32 をリトルエンディアンで付与）。
- 認証成功（0x76 ステータス 0x06）後に 0x33 ポーリングを開始。空欄なら認証をスキップします。セキュリティ OFF 機では 0x33 の REFUSE が返るまで 0x76 を送らないため、突然の切断を避けられます。
- 0x76 応答が 3 秒間返らない場合や 0x33 に応答が無い場合は write characteristic を 0x0003 → 0x0002 → 0x0007 の順で自動切り替えして再送するフェールバックを実装済み。無応答個体の切り分けに活用してください。
