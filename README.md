...
### セッションID設計のリファレンス / Session ID Reference

オフラインでCSVを保存して後からクラウドへアップロードする運用を安全にするため、**人間が読んで状況を推測しやすいセッションIDフォーマット**と同期フローをまとめたメモを `Docs/SessionIDDesign.md` に追加しています。セッションの複製・上書き・マージが発生する際のID構成や `Sync`/`Revision` の扱いを整理しているので、デバッグや問い合わせ対応時の指針として活用してください。
