...
//  PitTemp
//
//  役割: メタ入力、4輪×3ゾーンのセル、ライブグラフ、Export/Upload
//  初心者向けメモ:
//   - “UIは薄く/ロジックはVMへ”を意識（MVVM）
//   - 隠しHIDフィールドは overlay で右下に配置、デバッグ時のみ可視
//
...
struct MeasureView: View {
    ...
    @State private var showMetaEditor = false   // ← メタ編集シート

    var body: some View {
        NavigationStack {
            ScrollView {
                VStack(spacing: 16) {
                    headerReadOnly                     // ← ラベル表示
                    grid
                    if vm.isCaptureActive {
                        VStack(alignment: .leading, spacing: 6) {
                            Text("Live Temp (last \(Int(vm.chartWindowSec))s)")
        ...
        .onAppear {
            speech.requestAuth()
            // 画面復帰時：計測継続を許す（必要なければここで isCaptureActive を false に）
        }
        .onDisappear {
            // ★ 画面離脱時に HID 由来の取り込みは確実に停止
            vm.stopAll()
        }
    }
    ...
    // MeasureView.swift の header を差し替え
    ...
    private func metaRow(_ title: String, _ value: String) -> some View {
        HStack(spacing: 8) {
            ...
            Text(value.isEmpty ? "-" : value)
                .font(.headline)
                .textSelection(.enabled) // コピーだけはOK
            Spacer()
        }
    }
    ...
    private func wheelCard(_ w: WheelPos) -> some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            HStack {
                ...
                if speech.isRecording && speech.currentWheel == w {
                    ...
                    Button("Stop") {
                        ...
                        if !t.isEmpty { vm.appendMemo(t, to: w) }
                    }
                    ...
                    Button {
                        ...
                        if let pw = prevWheel, !prevText.isEmpty {
                            vm.appendMemo(prevText, to: pw)   // ★ 前回分を救済して保存
                        }
                        try? speech.start(for: w)            // 新規開始
                        Haptics.impactLight()
                    } label: {
            ...
            HStack(spacing: 8) {
                // 実際の位置関係を考慮：左輪= OUT-CL-IN / 右輪= IN-CL-OUT
    ...
    private func zoneButton(_ w: WheelPos, _ z: Zone) -> some View {
        Button { vm.tapCell(wheel: w, zone: z); focusTick &+= 1 } label: {
            ZStack {
            ...
            .overlay(alignment: .bottomLeading) {
                if vm.currentWheel == w && vm.currentZone == z {
                    let p = min(vm.elapsed / Double(vm.durationSec), 1.0)
                    ...
                    HStack(spacing: 6) {
                        ...
                        Text(String(format: "%.1fs", max(0, Double(vm.durationSec)-vm.elapsed)))
                            .font(.caption2).monospacedDigit()
                    }.padding(.top, 6)
                }
            }
            .overlay {
                // 数字は“はみ出しを恐れず大きく”、ゾーンで少し縦位置をずらす
                let y: CGFloat = (z == .OUT ? -18 : (z == .CL ? 0 : 18))
                Text(displayValue(w: w, z: z))
                    .font(.system(size: 40, weight: .semibold, design: .rounded))
                    .monospacedDigit().lineLimit(1).minimumScaleFactor(0.3)
                    .offset(y: y).allowsHitTesting(false)
            }
        }
        .buttonStyle(.plain)
    }
    ...
    private func title(_ w: WheelPos) -> String {
        switch w { case .FL: "Front Left"; case .FR: "Front Right"; case .RL: "Rear Left"; case .RR: "Rear Right" }
    }
    private func displayValue(w: WheelPos, z: Zone) -> String {
        if let r = vm.results.first(where: { $0.wheel == w && $0.zone == z }) { return r.peakC.isFinite ? String(format: "%.1f", r.peakC) : "--" }
    ...
    private var bottomBar: some View {
        HStack(spacing: 12) {
            Button(showRaw ? "Hide Raw" : "Show Raw") { showRaw.toggle(); focusTick &+= 1 }
                .buttonStyle(.bordered)
            Button("Stop") { vm.stopAll() }
                .buttonStyle(.bordered)
            Spacer()
//            Button("Next") { vm.userNext() }
            Button("Next") { vm.receiveSpecial("<RET>") }
                .buttonStyle(.bordered)

            // Export → iCloud Upload（選択済みフォルダへコピー）
            Button {
                if let url = vm.ensureCSV() {
                    folderBM.upload(file: url)   // 下の FolderBookmark に実装
                }
            } label: {
